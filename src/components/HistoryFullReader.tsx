    ...
    <div className="w-full space-y-24">
      {chapters.map((chapter: any, index: number) => (
        <div 
          key={chapter.id} 
          id={chapter.id}
          className="bg-white dark:bg-[#1a2024] rounded-[2.5rem] p-8 md:p-16 shadow-2xl border border-gray-50 dark:border-gray-800 relative overflow-hidden min-h-[400px] scroll-mt-24"
        >
          ...
          <div className="relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center gap-4 mb-8">
              ...
              <span className="text-accent-gold font-bold uppercase tracking-widest text-xs">
                {t('historyPage.chaptersBadge') || 'अध्याय'} {index + 1}
              </span>
            </div>
              ...
              {chapter.aarti && (
                <div className="my-16 p-8 md:p-12 bg-[#f4f7f7] dark:bg-black/20 rounded-3xl border-l-8 border-accent-gold relative">
                   ...
                   <div className="space-y-4">
                      {chapter.aarti.map((line: string, idx: number) => (
